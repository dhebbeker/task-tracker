 ...
 * "arduino-esp32/libraries/WebServer/examples/FSBrowser" to expose on-board
 * external Flash as USB Mass Storage and webserver. Both interfaces can make
 ...
 *   - you may need to format the drive as FAT. Note: If your PC failed to
 * format, you could format it using follow sketch
 * "https://github.com/adafruit/Adafruit_SPIFlash/tree/master/examples/SdFat_format"
 *   - Copy all files in 'data/' folder of this example to the root directory of
 * the MassStorage disk drive
 * 5. When prompted, open http://esp32fs.local/edit to access the file browser
 * 6. Modify/Update USB drive then refresh your browser to see if the change is
 * updated
 * 7. Upload/Edit a file using web browser then see if the USB Drive is updated.
 * Note: the usb drive could briefly disappear and reappear to force PC to
 * refresh its cache
 *
...
void handleFileUpload() {
  ...
  if (upload.status == UPLOAD_FILE_START) {
    ...
    DBG_SERIAL.print("handleFileUpload Name: ");
    DBG_SERIAL.println(filename);
    ...
    DBG_SERIAL.print("handleFileUpload Size: ");
    DBG_SERIAL.println(upload.totalSize);
  }
}
